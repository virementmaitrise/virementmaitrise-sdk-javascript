...
export const getInstance = (config: IHttpConfig) => {
    return axios.create({
        headers: {
            ...
            'User-Agent': `Virement Maitrisé NodeJS SDK v ${Constants.FINTECTURESDKVERSION}`,
            Authorization: `Basic ${config.clientToken}`,
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        baseURL:
            config.env === Constants.SANDBOXENVIRONMENT
                ? BaseUrls.FINTECTUREOAUTHURL_SBX
                : BaseUrls.FINTECTUREOAUTHURL_PRD,
