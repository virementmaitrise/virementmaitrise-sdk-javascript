...
Virement Maitrisé is a licensed and one-stop shop gateway to Open Banking.
...
Virement Maitrisé APIs enable to connect to both Production and Sandbox environments of banks. Our Sandbox has the particularity of being connected to other banks' Sandbox. This will give you a flavour of what you can expect in production in terms of user experience and data sets.

## Installation

```javascript
npm install 'virementmaitrise-client'
```

## Usage

The SDK enables multiple use cases. Essentially, it runs down to whether you want / need to go through our bank selection module "Virement Maitrisé Connect" ( which is mandatory if you're not an Agent or a licensed TPP. )
...
Independantly of the use case, the first step is to create a developer account on the [Virement Maitrisé Console](https://console.virementmaitrise.societegenerale.eu/auth/register) and create an application to get your **app_id**, **app_secret** and **private_key**.

You are now ready to start coding. Next step is
import the Virement Maitrisé Client library and instantiate the client object:

```javascript
const { FintectureClient } = require('virementmaitrise-client');
...
3. Authenticate your app to Virement Maitrisé and get your **access_token** and **refresh_token**
...
4. Authenticate your app to Virement Maitrisé and get your **access_token** and **refresh_token**
...
// get the Virement Maitrisé access token to request the AIS APIs
...
2. Authenticate your app to Virement Maitrisé and get your **access_token**
...
The initiate a payment on behalf of a PSU using Virement Maitrisé Connect, just do:
...
-   origin_uri: [optional] a URL to which the customer will be redirected if he wants to exit Virement Maitrisé Connect
...
-   payment_methods: [optional] A list of payment methods to be added to the selector on Connect. Example: ['immediate_transfer', 'smart_transfer']. Available values can be found on [docs](https://doc.virementmaitrise.societegenerale.eu/reference/createpisv2connect) in meta section.
